...
//       console.log(`Analyzing batch ${Math.floor(i/batchSize) + 1} of ${Math.ceil(allSearchResults.length/batchSize)}`);
      
//       try {
//         const batchAnalysis = await analyzeBatch(batch, Math.floor(i/batchSize) + 1);
...
//         }
//       } catch (openaiError) {
//         console.error(`OpenAI API Error for batch ${Math.floor(i/batchSize) + 1}:`, openaiError.message);
//         // Continue with other batches even if one fails
//         allAnalyses.push({
//           summary: `Analysis failed for batch ${Math.floor(i/batchSize) + 1}`,
//           ranked_list: [],
//           top_priority: [],
//           checklist: [],
//           next_actions: [],
...
//       searchResults: allSearchResults, // Return all 100 results
...
//   const uniqueTopPriority = [...new Set(allTopPriority)].slice(0, 10); // Limit to top 10
  
//   // Take checklist and next_actions from the first analysis (they're usually similar)
//   const checklist = analyses[0]?.checklist || [];
//   const next_actions = analyses[0]?.next_actions || [];
//   const disclaimer = analyses[0]?.disclaimer || "This is an automated risk-assessment, not legal advice; consult counsel before taking legal action.";
...
//   const summary = `Analyzed ${totalVideos} videos across ${analyses.length} batches. Found ${totalHighRisk} high-risk, ${totalMediumRisk} medium-risk, and ${totalLowRisk} low-risk videos. ${totalHighRisk > 0 ? 'Immediate attention recommended for high-risk content.' : 'No immediate high-risk concerns detected.'}`;
...
function combineAnalyses(analyses, totalVideos) {
  ...
  const summary = `Analyzed ${totalVideos} videos across ${analyses.length} batches${failedBatches > 0 ? ` (${failedBatches} batches failed)` : ''}. Found ${totalHighRisk} high-risk, ${totalMediumRisk} medium-risk, and ${totalLowRisk} low-risk videos. ${totalHighRisk > 0 ? 'Immediate attention recommended for high-risk content.' : 'No immediate high-risk concerns detected.'}`;
